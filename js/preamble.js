...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const KEYWORDS = [
        '人民', '民主', '法治', '社会主义', '现代化',
        '中华民族伟大复兴', '统一', '团结', '和平发展',
        '人类命运共同体', '改革开放', '创新', '和谐', '美丽'
    ];

    // 直接嵌入宪法序言内容
    const PREAMBLE_DATA = {
        year: 1982,
        name: "中华人民共和国宪法（1982年）",
        paragraphs: [
    ...
    function initPreamblePage() {
        renderPreamble(PREAMBLE_DATA);
        renderParagraphNav(PREAMBLE_DATA);
        initScrollObserver();
        initScrollSync();
    ...
    function renderPreamble(data) {
        const container = document.getElementById('preambleContent');
        if (!container) return;

        const html = data.paragraphs.map((paragraph, index) => {
            const highlightedText = applyKeywordHighlight(paragraph);
            return `
                <div class="preamble-paragraph" id="paragraph-${index + 1}">
                    <span class="paragraph-number">${index + 1}</span>
                    <p class="preamble-paragraph-text">${highlightedText}</p>
                </div>
            `;
        }).join('');
    ...
    function renderParagraphNav(data) {
        const navContainer = document.getElementById('paragraphNav');
        if (!navContainer) return;

        const html = data.paragraphs.map((_, index) => {
            const label = `第 ${index + 1} 段`;
            return `<li><a href="#paragraph-${index + 1}" data-target="paragraph-${index + 1}">${label}</a></li>`;
        }).join('');
    ...
    function scrollToParagraph(targetId) {
    ...
    function initScrollObserver() {
        ...
        document.querySelectorAll('.preamble-paragraph').forEach(paragraph => {
    ...
    function initScrollSync() {
        ...
        window.addEventListener('scroll', () => {
            ...
            scrollTimeout = setTimeout(updateActiveNavByScroll, 30);
        }, { passive: true });
    }

    function updateActiveNavByScroll() {
        const paragraphs = document.querySelectorAll('.preamble-paragraph');
        const navbarHeight = document.getElementById('navbar')?.offsetHeight || 64;
        ...
        paragraphs.forEach((p) => {
            ...
            const visibleTop = Math.max(0, -paragraphTop);
            const visibleBottom = Math.min(viewportHeight - navbarHeight, viewportHeight - paragraphTop);
            const visibleHeight = Math.max(0, visibleBottom - visibleTop);
            const visibilityRatio = visibleHeight / paragraphHeight;
